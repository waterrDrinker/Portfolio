...
import { Navigation, NavItemId, NavSocialId } from '@/shared/types/navigation';
...
const Header: FC<HeaderProps> = ({ lang, navigation }) => {
  const LogoElement = (
    <ButtonLink className={styles.logoLink} href="/" variant="ghost">
          ...
          <nav className={clsx(styles.layoutLeft)}>
            <ul className={styles.list}>
              ...
              {navigation.primary.map((item, i) => {
                if (item.id === NavItemId.Home) return null;
            ...
            <nav className={styles.socialLinks}>
              <ul className={styles.list}>
                {navigation.socials.map((item, i) => {
                  if (item.id === NavSocialId.Gmail) return null;
